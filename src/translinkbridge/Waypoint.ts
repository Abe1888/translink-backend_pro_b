...
/** Resolves overlay colors dynamically based on the current active theme */
function getOverlayColors() {
    const isDark = document.documentElement.classList.contains('dark');
    return {
        titleColor:   isDark ? '#ffffff'                    : '#161616',
        subtitleColor: isDark ? 'rgba(255,255,255,0.6)'    : 'rgba(22,22,22,0.7)',
        descColor:    isDark ? 'rgba(255,255,255,0.75)'    : 'rgba(22,22,22,0.85)',
        statsColor:   isDark ? '#ffffff'                   : '#161616',
        dividerColor: isDark ? 'rgba(255,255,255,0.2)'     : 'rgba(22,22,22,0.3)',
        dotColor:     'var(--brand-crimson)',
        dotShadow:    'rgba(192,32,47,0.35)',
        ringColor:    'rgba(192,32,47,0.4)',
        pathStroke:   isDark ? '#ffffff'                   : 'rgba(22,22,22,0.45)',
        endpointFill: isDark ? '#ffffff'                   : '#161616',
        cardBg:       'transparent',
    };
}
...
export class Waypoint {
    ...
    buildDOM(): void {
        ...
        this.configs.forEach((config) => {
            ...
            const isDark = document.documentElement.classList.contains('dark');
            const pathStroke = isDark ? '#ffffff' : (isContactType ? '#161616' : 'rgba(22, 22, 22, 0.35)');
            ...
            endpoint.setAttribute('fill', getOverlayColors().endpointFill);
    ...
    private updateOverlayColors(data: WaypointOverlayData): void {
        // Resolve colors fresh each call so theme switches apply immediately
        ...
        // Update SVG path + endpoint color
        if (data.path) {
            data.path.setAttribute('stroke', colors.pathStroke);
        }

        if (data.endpoint) {
            data.endpoint.setAttribute('fill', colors.endpointFill);
        }

        // Ensure card background stays fully transparent
        if (data.card) {
            data.card.style.background = 'transparent';
            data.card.style.backdropFilter = 'none';
            (data.card.style as any).webkitBackdropFilter = 'none';
            data.card.style.boxShadow = 'none';
        }
