...
export class TranslinkSoundToggle {
    ...
    mount(parent: HTMLElement): void {
        ...
        this.button.innerHTML = `
            <div class="sound-icon-container w-[30px] h-[30px] flex items-center justify-center rounded-full border border-black/10 dark:border-white/10 bg-[#f5f1e8]/70 dark:bg-black/40 shadow-sm backdrop-blur-sm transition-all duration-300 group-hover:bg-[#f5f1e8] dark:group-hover:bg-black group-hover:border-black/25 dark:group-hover:border-white/20">
                <svg class="sound-waves w-4 h-4 transition-all duration-500 ease-out" viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M2 10v4"></path>
                    <path d="M6 7v10"></path>
    ...
    private updateButtonState(): void {
        ...
        if (this.isPlaying) {
            ...
            if (waves) {
                waves.style.stroke = 'var(--theme-h-color)';
            ...
            if (waves) {
                waves.style.stroke = 'var(--theme-wp-dot)';
