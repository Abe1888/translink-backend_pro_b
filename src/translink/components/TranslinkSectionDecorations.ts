    ...
    mount(parent: HTMLElement): void {
        ...
        if (this.dec.showHVLines || this.dec.showGeo) {
            ...
            if (this.dec.showGeo) {
                ...
                // Create the dashboard technical grid-anchor outline box
                const geo2 = document.createElement('div');
                geo2.id = `${this.id}-geo2`;
                geo2.className =
                    'absolute w-[12vh] h-[12vh] top-[60%] left-[10%] rotate-45 border border-dashed flex items-center justify-center bg-transparent deco-shape-solid';
                geo2.style.borderColor = 'var(--theme-border-color)';
                geo2.style.opacity = '0.5';
                geo2.style.background = 'transparent';
                geo2.innerHTML = `<div class="w-1.5 h-1.5 animate-ping rounded-full" style="background-color: var(--theme-wp-dot, #c0202f); opacity: 0.6;"></div>`;
