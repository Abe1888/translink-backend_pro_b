...
export class TranslinkAIBrain {
    ...
    constructor() {
        ...
        // Cache coordinates initially and set up lifecycle hooks
        this._cacheSectionOffsets();
        if (document.readyState === 'complete') {
            this._cacheSectionOffsets();
        } else {
            window.addEventListener('load', () => this._cacheSectionOffsets(), { once: true });
        }
        let resizeTimeout: ReturnType<typeof setTimeout>;
        window.addEventListener('resize', () => {
            clearTimeout(resizeTimeout);
            resizeTimeout = setTimeout(() => this._cacheSectionOffsets(), 250);
        }, { passive: true });
...
${userQuery ? `The visitor just said: "${userQuery}". Respond directly, professionally, and with emotional intelligence.` : 'New context received. Process quietly. Only speak if the timing is genuinely helpful.'}`;
    ...
    private _cacheSectionOffsets(): void {
        const sections = ['s1', 's2', 's3', 's4', 's5', 's6', 's7', 's8', 's9', 's10'];
        sections.forEach((id) => {
            const el = document.getElementById(id);
            if (el) {
                const rect = el.getBoundingClientRect();
                this.sectionOffsets[id] = rect.top + window.scrollY;
            }
        });
    }
    ...
    private _getCurrentSectionFromScroll(): string {
        ...
        sections.forEach((id) => {
            const offset = this.sectionOffsets[id];
            if (offset !== undefined) {
                const dist = Math.abs(offset - currentScrollY);
                if (dist < minDistance) {
                    minDistance = dist;
                    closestSection = id;
                }
