...
export class TranslinkRobotLoader {
    ...
    mount(): void {
        ...
        this.overlay.className =
            'fixed inset-0 w-full h-full bg-[var(--theme-bg-color)] z-[99999] flex flex-col items-center justify-center transition-all duration-1000 ease-in-out overflow-hidden';

        // Cyber-Premium Loader with Holographic elements
        this.overlay.innerHTML = `
            <!-- Animated Grid Background -->
            <div class="absolute inset-0 opacity-[var(--theme-line-opacity)] pointer-events-none">
                <div class="absolute inset-0 bg-[linear-gradient(90deg,var(--theme-line-color)_1px,transparent_1px),linear-gradient(var(--theme-line-color)_1px,transparent_1px)] bg-[size:40px_40px] [mask-image:radial-gradient(ellipse_50%_50%_at_50%_50%,#000_70%,transparent_100%)]"></div>
            </div>
                ...
                <div class="flex flex-col items-center w-full relative mt-2 gap-3 md:mt-4 md:gap-4">
                    ...
                    <div class="relative px-6 py-2 md:px-10 md:py-4 flex flex-col items-center">
                        ...
                        <div class="mt-2 md:mt-4 flex items-center gap-3 opacity-80">
                            ...
                            <span id="loader-status" class="font-mono text-[9px] md:text-[10px] tracking-[0.2em] uppercase text-[var(--theme-h-color)] font-bold">${lang.t('global.loader_core_init')}</span>
                            <div class="flex gap-1"><div class="w-1.5 h-1.5 bg-crimson"></div><div class="w-1.5 h-1.5 bg-crimson"></div></div>
                        </div>
                    </div>
                    
                    <!-- Title Text Frame -->
                    <div class="relative w-full max-w-[340px] border border-[var(--theme-border-color)] bg-[var(--theme-card-bg)] backdrop-blur-sm p-4 md:p-5 flex flex-col items-center gap-2 md:gap-3 mt-1 md:mt-2" style="clip-path: polygon(15px 0, 100% 0, 100% calc(100% - 15px), calc(100% - 15px) 100%, 0 100%, 0 15px);">
                        ...
                        <div class="text-center font-mono font-bold tracking-widest uppercase text-[10px] md:text-sm text-[var(--theme-h-color)] flex items-center gap-2 md:gap-3 opacity-80">
    ...
    /**
     * Gracefully hide and remove the loader automatically
     */
    async hide(): Promise<void> {
        ...
        // Wait for the visual count-up animation to complete to 100%
        while (this.currentDisplayPercent < 100) {
            await new Promise((resolve) => requestAnimationFrame(resolve));
        }
        await new Promise((resolve) => setTimeout(resolve, 300));

        // Play a premium high-tech entry beep cue if allowed
        try {
        ...
        // Unmute/resume any active audio contexts
        window.dispatchEvent(new CustomEvent('translink:audio-unlock'));
